...
do
    ...
    for i in 1 8 27 64
    do
        likwid-mpirun --debug --np $i -g MEM_DP -mpi slurm --mpiopts "--cpu-freq=$f-$f:performance" -nperdomain N:$i -m -O ../../build_MPI_SPR2/lulesh2.0 -p -b 0 -s ${size} -i ${iter} -c 0 >> MEM_DP_$i.csv
    done
done
