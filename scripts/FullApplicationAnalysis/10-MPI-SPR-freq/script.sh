...
do
    ...
    for i in 1 8 27 64; do
        likwid-mpirun --debug -mpi slurm --mpiopts "--cpu-freq=$f-$f:performance" --np $i -nperdomain N:$i -g MEM_DP -O ../../../build/build_MPI_SPR/lulesh2.0 -p -b 0 -s ${size} -i ${iter} -c 0 >> size_${size}/MEM_DP_$i.csv
    done
done
